...
13. Run `npm run patch-filter:validate`; rename, generated-file, one-hop context, omitted-region, deterministic, and full-diff policy-preservation cases must remain explicit.
14. Run `npm run remediation-suggestions:validate`; default-off, stale, ambiguous, ownerless, security-sensitive, unsupported, and resource-bound cases must remain explicit.
15. Run `npm run remediation-rules:validate`; every reviewed rule must retain applicability, preconditions, non-goals, validation descriptions, and golden positive/negative fixtures.
